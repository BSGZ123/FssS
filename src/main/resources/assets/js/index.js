...
function getUserDownloaded() {  //获取用户已下载的记录
...
function getUserUploaded() {   //获取用户自有上传的文件记录
...
function setResources(resources, tabId) {   //这个函数用于填充页面数据
    var contentHtml = "";
    search = "";
    if (resources.length < 1) {
        ...
        $.each(resources, function (i, resource) {
            ...
            contentHtml += "<div class='row content-box rounded' data-id='" + resource.id + "'><div class='col-12 col-sm-12'><br/><div class='row'>" +
                ...
                ("#uploaded-content" === tabId ? "&emsp;<a href='javascript:;' class='font-1' onclick='viewFile();'>在线预览</a>&emsp;<a href='javascript:;' class='font-1' onclick='editFile();'>编辑</a>&emsp;<a href='javascript:;' class='font-1' onclick='removeFile();'>删除</a>" : "") +
...
function viewFile() {
    ...
    window.open('http://localhost:8012/onlinePreview?url='+encodeURIComponent(fileUrl));
    // alerts(fileUrl);
